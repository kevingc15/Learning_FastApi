...
app = FastAPI()
oauth2 = OAuth2PasswordBearer(tokenUrl="login", scopes={"me": "Read information about the current user.", "items": "Read items."})
...
async def get_current_user(security_scopes: SecurityScopes, token: Annotated[str, Depends(oauth2)]):
    ...
    try:
        ...
        token_data = Token_data(scopes=token_scopes, username=username)
...
async def login(form: Annotated[OAuth2PasswordRequestForm, Depends()]):
    ...
    jwt_token = create_access_token(data={"sub": user_db.username, "scopes": " ".join(form.scopes)})

    return {"access_token": jwt_token, "token_type": "bearer"}


@app.get("/users/me")
async def me(current_user: Annotated[User, Security(get_current_user, scopes=["me"])]):
    return current_user
